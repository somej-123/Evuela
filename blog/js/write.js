...
$(document).ready(() => {
        ...
        if (
          ...
          for (let i = 0; i < data.length; i++) {
            if (data[i].board_category_parents_idx == null) {
              $("#writeHeader_category").append(
                '<option class="write_category_option" value=' +
  ...
  $("#summernote_submit_btn").on("click", () => {
    ...
    let board_contents = $("#mainSection_Body_write_summernote").summernote(
      "code"
    );
    ...
    if (user_id == null || user_id == "" || user_id == undefined) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          // 이미지 태그가 없는 경우 등록된 이미지 전부 삭제
          if (imgTags.length == 0) {
            $.ajax({
              url: "./ajax/deleteBoardImgAll.php",
              async: false,
              data: {
                board_id: board_id,
              },
          ...
          if (imgTags.length > 0) {
            for (; imgCntUpdates < imgTags.length; imgCntUpdates++) {
              let imgTagID = imgTags.eq(imgCntUpdates).attr("id");
              imgTagIDArray.push(imgTagID);
              // 이미지 크기 수정
              let imgTagWidth = imgTags
                .eq(imgCntUpdates)
                .css("width")
                .replace("px", "");
              let imgTagHeight = imgTags
                .eq(imgCntUpdates)
                .css("height")
                .replace("px", "");
...
function setImgFile(files, editor) {
  ...
  for (var i = 0; i < files.length; i++) {
    if (!files[i].name.match(reg)) {
      showAlert(
        "이미지는 gif, jpg, png, jepg 파일만 업로드가 가능합니다.",
        "error"
      );
      imgFilecheck = false;
      return;
